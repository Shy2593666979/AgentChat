...
class MCPUserConfigService:
    ...
    @classmethod
    async def update_mcp_user_config(cls, config_id: str, mcp_server_id: str,
                                     user_id: str, config: Optional[dict] = None):
        """
        更新MCP用户配置记录。
        :param config_id: 配置记录ID
        :param mcp_server_id: MCP Server ID
        ...
        """
        try:
            return await MCPUserConfigDao.update_mcp_user_config(config_id, mcp_server_id, user_id, config)
        except Exception as err:
